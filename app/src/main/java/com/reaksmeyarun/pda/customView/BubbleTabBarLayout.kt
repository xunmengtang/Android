...
import com.reaksmeyarun.pda.customView.BubbleTabBar.Bubble
import com.reaksmeyarun.pda.customView.BubbleTabBar.parser.MenuParser

    class BubbleTabBarLayout : LinearLayout {
    var onBubbleClickListener : OnBubbleClickListener? = null
    var disabledIconColorParam : Int = Color.GRAY
    var horizontalPaddingParam : Float = 0F
    var iconPaddingParam : Float = 0F
    var verticalPaddingParam : Float = 0F
    var iconSizeParam : Float = 0F
    var titleSizeParam: Float = 0F
    var customFontParam: Int = 0
    ...
    fun setSelected(position: Int, callListener: Boolean = true) {
        val it = (this@BubbleTabBarLayout.getChildAt(position) as Bubble)
    ...
    private fun init(
        ...
        if (attrs != null) {
            val attributes =
                context.theme.obtainStyledAttributes(attrs, R.styleable.BubbleTabBarLayout, 0, 0)
            try {
                val menuResource = attributes.getResourceId(R.styleable.BubbleTabBarLayout_bubbletab_menuResource, -1)
                disabledIconColorParam = attributes.getColor(
                    R.styleable.BubbleTabBarLayout_bubbletab_disabled_icon_color,
                    Color.GRAY
                )
                customFontParam =
                    attributes.getResourceId(R.styleable.BubbleTabBarLayout_bubbletab_custom_font, 0)

                iconPaddingParam = attributes.getDimension(
                    R.styleable.BubbleTabBarLayout_bubbletab_icon_padding,
                    resources.getDimension(R.dimen.bubble_icon_padding)
                )
                horizontalPaddingParam = attributes.getDimension(
                    R.styleable.BubbleTabBarLayout_bubbletab_horizontal_padding,
                    resources.getDimension(R.dimen.bubble_horizontal_padding)
                )
                verticalPaddingParam = attributes.getDimension(
                    R.styleable.BubbleTabBarLayout_bubbletab_vertical_padding,
                    resources.getDimension(R.dimen.bubble_vertical_padding)
                )
                iconSizeParam = attributes.getDimension(
                    R.styleable.BubbleTabBarLayout_bubbletab_icon_size,
                    resources.getDimension(R.dimen.bubble_icon_size)
                )
                titleSizeParam = attributes.getDimension(
                    R.styleable.BubbleTabBarLayout_bubbletab_title_size,
                    resources.getDimension(R.dimen.bubble_icon_size)
                )
                if (menuResource >= 0) {
